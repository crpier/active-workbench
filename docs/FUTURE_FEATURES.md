...
- `Expo app + Android native reliable share ingest` (planned)
  - Goal: keep a single Expo-based app/UI codebase, while adding Android-native background reliability for shared-article capture.
  - Practical architecture: retain Expo/React screens and state; add a focused Android module (WorkManager + share receiver + persisted queue) bridged to JS for enqueue/status/flush events.
  - Why: preserve cross-platform/product UI consistency while restoring stronger delivery guarantees under process death/offline/retry scenarios.
  - Scope note: iOS parity is not required for this iteration.
