...
- Optional BookWyrm book-enrichment knobs:
  - `ACTIVE_WORKBENCH_BUCKET_BOOKWYRM_BASE_URL` (default `https://bookwyrm.social`)
  - `ACTIVE_WORKBENCH_BUCKET_BOOKWYRM_USER_AGENT` (identify app + contact URL/email)
  - `ACTIVE_WORKBENCH_BUCKET_BOOKWYRM_DAILY_SOFT_LIMIT` (default `500`)
  - `ACTIVE_WORKBENCH_BUCKET_BOOKWYRM_MIN_INTERVAL_SECONDS` (default `1.1`)
- Optional MusicBrainz music-album enrichment knobs:
  - `ACTIVE_WORKBENCH_BUCKET_MUSICBRAINZ_BASE_URL` (default `https://musicbrainz.org`)
  - `ACTIVE_WORKBENCH_BUCKET_MUSICBRAINZ_USER_AGENT` (identify app + contact URL/email)
  - `ACTIVE_WORKBENCH_BUCKET_MUSICBRAINZ_DAILY_SOFT_LIMIT` (default `500`)
  - `ACTIVE_WORKBENCH_BUCKET_MUSICBRAINZ_MIN_INTERVAL_SECONDS` (default `1.1`)
