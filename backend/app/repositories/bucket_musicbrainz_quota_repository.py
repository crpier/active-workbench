...
class BucketMusicbrainzQuotaSnapshot:
    date_utc: str
...
class BucketMusicbrainzQuotaRepository:
        ...
        *,
    ...
    ) -> BucketMusicbrainzQuotaSnapshot:
        ...
        with self._db.connection() as conn:
            row = conn.execute(
                ...
                FROM bucket_musicbrainz_quota_daily
                WHERE date_utc = ?
                """,
                (date_utc,),
            ).fetchone()
            ...
            if min_interval > 0 and calls_today > 0 and last_updated_at is not None:
                ...
                if elapsed_seconds < min_interval:
                    ...
                    return BucketMusicbrainzQuotaSnapshot(
                        date_utc=date_utc,
            ...
            if row is None:
                conn.execute(
                    """
                    INSERT INTO bucket_musicbrainz_quota_daily (date_utc, calls, updated_at)
                    VALUES (?, 1, ?)
                    """,
                    (date_utc, now_iso),
                )
                ...
                conn.execute(
                    """
                    UPDATE bucket_musicbrainz_quota_daily
