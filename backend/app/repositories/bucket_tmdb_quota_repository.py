...
class BucketTmdbQuotaRepository:
        ...
        with self._db.connection() as conn:
            ...
            if min_interval > 0 and calls_today > 0 and last_updated_at is not None:
                ...
                if elapsed_seconds < min_interval:
                    ...
                    return BucketTmdbQuotaSnapshot(
                        date_utc=date_utc,
            ...
            if row is None:
                conn.execute(
                    """
                    INSERT INTO bucket_tmdb_quota_daily (date_utc, calls, updated_at)
                    VALUES (?, 1, ?)
                    """,
                    (date_utc, now_iso),
                )
                ...
                conn.execute(
                    """
                    UPDATE bucket_tmdb_quota_daily
